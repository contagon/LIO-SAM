...
public:
  ...
  // TODO: I think I should be able to remove this, it's only used for
  ...
  MapOptimization(LioSamParams &params) : params_(params) {
    ISAM2Params parameters;
    parameters.relinearizeThreshold = 0.1;
    parameters.relinearizeSkip = 1;
    isam = new ISAM2(parameters);

    // TODO: Outputs if we need them
    // pubKeyPoses =
    //     nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/trajectory",
    //     1);
    // pubLaserCloudSurround =
    //     nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/map_global",
    //     1);
    ...
    // pubRecentKeyFrames =
    //     nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/map_local",
    //     1);
    ...
    downSizeFilterSurf.setLeafSize(params_.mappingSurfLeafSize,
                                   ...
                                   params_.mappingSurfLeafSize);
    downSizeFilterICP.setLeafSize(params_.mappingSurfLeafSize,
                                  params_.mappingSurfLeafSize,
                                  params_.mappingSurfLeafSize);
  ...
  void laserCloudInfoHandler(const CloudInfo<PointType> &cloudInfo) {
    // extract time stamp
    ...
    if (timeLaserInfoCur - timeLastProcessing >=
        params_.mappingProcessInterval) {
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void extractNearby() {
    ...
    kdtreeSurroundingKeyPoses->radiusSearch(
        cloudKeyPoses3D->back(),
        (double)params_.surroundingKeyframeSearchRadius, pointSearchInd,
        pointSearchSqDis);
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void transformUpdate(const CloudInfo<PointType> &cloudInfo) {
    if (cloudInfo.imuAvailable == true) {
      if (std::abs(cloudInfo.imuPitchInit) < 1.4) {
        ...
        // slerp roll
        transformQuaternion =
            rpy2quat(double(transformTobeMapped[0]), 0.0, 0.0);
        ...
        // slerp pitch
        transformQuaternion =
            rpy2quat(0.0, double(transformTobeMapped[0]), 0.0);
  ...
  void publishOdometry(const CloudInfo<PointType> &cloudInfo) {
